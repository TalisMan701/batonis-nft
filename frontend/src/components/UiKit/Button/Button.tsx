...
type IButtonAsidePropsWithType =
    | {type?: 'button'; href?: string; target?: string}
    | {type?: 'link'; href: string; target: '_blank' | '_self' | '_parent' | '_top'};

type MainProps = IButtonAsideProps & IButtonAsidePropsWithIcon & IButtonAsidePropsWithType;

const Button: FC<MainProps> = ({
    label,
    style,
    icon,
    iconDirection,
    ...
    const classNames = `${classes.btn} ${size === 'l' ? classes.btnL : ''} ${
        size === 's' ? classes.btnS : ''
    } ${color === 'secondary' ? classes.secondary : ''} ${!label && icon ? classes.onlyIcon : ''}`;

    const LinkOrButton: FC<{children: React.ReactNode; type: 'link' | 'button'}> = ({
        children,
        type,
    }) => {
        if (type === 'button') {
    ...
    if (label) {
        return (
            <LinkOrButton type={type}>
                <>
                    {icon && iconDirection === 'left' && icon}
                    <span
                        style={{
                            marginLeft: iconDirection === 'left' ? 12 : 0,
                            marginRight: iconDirection === 'right' ? 12 : 0,
                        }}
                    >
                        {label}
                    </span>
    ...
    return <LinkOrButton type={type}>{icon}</LinkOrButton>;
};
