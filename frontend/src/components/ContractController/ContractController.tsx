...
import {getContract, getFromContract, getPaymentContract, getPrice, saleIsOpen} from '../../api/web3';
    ...
    return (
        <ReactPortal wrapperId={'react-portal-contract-controller'}>
            <div className={classes.wrapper}>
                ...
                <>
                    {paymentToken &&
                        <div>Payment: {paymentToken}</div>
                    }
                    {priceMintNFT &&
                        <div>Price NFT: {priceMintNFT}</div>
                    }
                </>
                <div>
                    <input type="text" value={paymentTokenTemp} onChange={(e) => setPaymentTokenTemp(e.target.value)}/>
                    <div className={classes.btn} onClick={()=>{
                        if(paymentTokenTemp){
                            _setPaymentToken(paymentTokenTemp)
                        }
                    }}>Set payment token</div>
                </div>
                <div>
                    <input type="text" value={addressForGetTokens} onChange={(e) => setAddressForGetTokens(e.target.value)}/>
                    <div className={classes.btn} onClick={()=>{
                        if(addressForGetTokens){
                            _mintTokensForAddress(addressForGetTokens)
                        }
                    }}>Mint BTNT tokens for address</div>
                </div>
                <div>
                ...
                <div>
                    <input type="text" value={signSalt} onChange={(e) => setSignSalt(e.target.value)}/>
                    <div className={classes.btn} onClick={()=>{
                        if(signSalt){
