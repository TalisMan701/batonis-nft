...
export const _eventOnChangeChain = () => async (dispatch: AppDispatch) => {
    if (ethereum) {
        ...
        ethereum.on('chainChanged', async (chainId: string) =>
            dispatch(userSlice.actions.setIsRightChainId(await checkChainId(chainId))),
        );
    }
};
...
export const _mint = (currentAccount: string) => async (dispatch: AppDispatch) => {
    if (ethereum) {
        ...
        dispatch(
            userSlice.actions.setMintStage({
                stage: 'Get allowance token for contract',
                progress: 10,
            }),
        );
        ...
        const approveHandler = async () => {
            ...
            try {
                dispatch(
                    userSlice.actions.setMintStage({
                        stage: 'Approve token for contract',
                        progress: 30,
                    }),
                );
        ...
        if (allowance < price) {
            console.log('Approve');
            dispatch(
                userSlice.actions.setMintStage({stage: 'Approve token for contract', progress: 20}),
            );
